...
0x000000,0xFFFFFF,0xFFFFFF,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xC0C0C0,0xFFFFFF,0x000000};
        ...
        public static void CreateFolders()
        {
            ...
            //{
                ...
                FS.AddFile("FLOWDOS|0.5", "info.fd", "/sys");
            //}
...
Console.BackgroundColor = ConsoleColor.Black;
            ...
            if (devversion)
            {
                GruntyOS.CurrentUser.Username = "root";
            }
           ...
           /* unsafe
            ...
            Stream ballmer = Environment.RootFS.OpenFile("/dev/ballmer", (int)FileMode.ReadAndWrite);

            int dat2 = 0;
            while (dat2 != -1)
            {
                dat2 = ballmer.Read();
                Console.Write((char)dat2);
            }
           ...
           // alaffutdescommandes:
                ...
                Console.Write(GruntyOS.CurrentUser.Username + " ");
                //Console.Write("root ");
...
Console.Clear();
    ...
    public static byte[] ConvertStringToBytes(string input)
  {
    ...
    public string GetStrFromAttributes(byte Attributes)
    {
        return GetStrFromAttributes(int.Parse(Attributes.ToString()));
    }

    public string GetStrFromAttributes(int Attributes)
    {
        if (Attributes.ToString().Length == 1)
        {
            switch (Attributes)
            {
                case 0:
    ...
    public string GetCharStrFromAttributes(int Attributes)
    {
        if (Attributes.ToString().Length == 1)
        {
            switch (Attributes)
    ...
    hlp:
        ...
        else if (cmd.ToLower() == "cat" || cmd.ToLower() == "cat ")
        {
            if (string.IsNullOrWhiteSpace(args))
            {
                Console.BackgroundColor = ConsoleColor.Blue;
                Console.WriteLine("USAGE: cat <file to show>");
                Console.BackgroundColor = ConsoleColor.Black;
            }
            else
            {
        ...
        else if (cmd.ToLower() == "keytest")
        {
            while (true)
            {
                uint f = 0;
                Cosmos.Hardware.Global.Keyboard.GetScancode(out f);
                Console.WriteLine((int)f + "u");
                Console.Read();
            }
        }
        else if (cmd.ToLower() == "fileinfo" || cmd.ToLower() == "fileinfo " || cmd.ToLower() == "finfo" || cmd.ToLower() == "finfo ")
        {
            if (string.IsNullOrWhiteSpace(args))
            {
                Console.BackgroundColor = ConsoleColor.Blue;
                Console.WriteLine("USAGE: fileinfo <file to show>");
                Console.WriteLine("USAGE: finfo <file to show>");
                Console.BackgroundColor = ConsoleColor.Black;
            }
            else
            {
                Console.BackgroundColor = ConsoleColor.Blue;
                Console.WriteLine("Informations about file " + args);
                Console.BackgroundColor = ConsoleColor.Black;
                fsEntry f = fd.readFromNode(args);
                ...
                for (int j = 0; j < fd.getLongList("/").Length; j++)
                {
                    fsEntry f1 = fd.getLongList("/")[j];
                    if (f1.Name == s1)
                    {
                        f = f1;
                    }
                ...
                int owner_rights = int.Parse(f.Attributes.ToString()[0].ToString());
                int group_rights = int.Parse(f.Attributes.ToString()[1].ToString());
                int other_rights = int.Parse(f.Attributes.ToString()[2].ToString());

                string own = GetStrFromAttributes(owner_rights);
                string grp = GetStrFromAttributes(group_rights);
                string otr = GetStrFromAttributes(other_rights);

                string ownx = GetCharStrFromAttributes(owner_rights);
                string grpx = GetCharStrFromAttributes(group_rights);
                string otrx = GetCharStrFromAttributes(other_rights);

                Console.WriteLine("Your permissions: " + own + " (" + ownx + ")");
                dewitcher.Core.Memory.MemAlloc(sizeof(Int64));
                Console.WriteLine("Group permissions: " + grp + " (" + grpx + ")");
                Console.WriteLine("Other users permissions: " + otr + " (" + otrx + ")");

                Console.WriteLine("Checksum: " + f.Checksum);
                Console.WriteLine("Owner: " + f.Owner);
                Console.WriteLine("Last change time: " + f.Time);

            }
        }
        else if (cmd.ToLower() == "flowhash")
        {
            if (string.IsNullOrWhiteSpace(args))
            {
                Console.BackgroundColor = ConsoleColor.Blue;
                Console.WriteLine("USAGE: flowhash <text to hash>");
                Console.BackgroundColor = ConsoleColor.Black;
            }
            else
            {
                Console.WriteLine(dewitcher.Crypto.FlowHash.Hash(args));
            }
        }
        else if (cmd.ToLower() == "writeboot")
        {
            byte[] array1 = new byte[] { 0xe9, 0xbb, 0x00, 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64, 0x2c, 0x20, 0x62, 0x6f, 0x6f, 0x74, 0x69, 0x6e, 0x67, 0x20, 0x66, 0x72, 0x6f, 0x6d, 0x20, 0x6c, 0x6f, 0x77, 0x2d, 0x6c, 0x65, 0x76, 0x65, 0x6c, 0x20, 0x31, 0x36, 0x2d, 0x62, 0x69, 0x74, 0x2e, 0x2e, 0x2e, 0x0, 0x46, 0x69, 0x73, 0x6e, 0x69, 0x6b, 0x27, 0x73, 0x20, 0x6f, 0x77, 0x6e, 0x20, 0x62, 0x6f, 0x6f, 0x74, 0x6c, 0x6f, 0x61, 0x64, 0x65, 0x72, 0x20, 0x70, 0x72, 0x6f, 0x67, 0x72, 0x61, 0x6d, 0x20, 0x77, 0x72, 0x69, 0x74, 0x74, 0x65, 0x6e, 0x20, 0x69, 0x6e, 0x20, 0x78, 0x38, 0x36, 0x20, 0x61, 0x73, 0x73, 0x65, 0x6d, 0x62, 0x6c, 0x79, 0x20, 0x6c, 0x61, 0x6e, 0x67, 0x75, 0x61, 0x67, 0x65, 0x2e, 0x0, 0x50, 0x72, 0x65, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x79, 0x20, 0x6b, 0x65, 0x79, 0x20, 0x74, 0x6f, 0x20, 0x72, 0x65, 0x62, 0x6f, 0x6f, 0x74, 0x2e, 0x2e, 0x2e, 0x0, 0xac, 0x8, 0xc0, 0x74, 0x7, 0xb4, 0xe, 0xcd, 0x10, 0xe9, 0xf4, 0xff, 0xe8, 0x0, 0x0, 0xb0, 0x0, 0xaa, 0xb4, 0xe, 0xb0, 0xd, 0xcd, 0x10, 0xb0, 0xa, 0xcd, 0x10, 0xc3, 0xbe, 0x73, 0x7c, 0xe8, 0xdd, 0xff, 0xe8, 0x5, 0x0, 0xea, 0x0, 0x0, 0xff, 0xff, 0xb4, 0x0, 0xcd, 0x16, 0xc3, 0xfa, 0x8c, 0xc8, 0x8e, 0xd8, 0x8e, 0xc0, 0x8e, 0xd0, 0xfb, 0xbe, 0x3, 0x7c, 0xe8, 0xc0, 0xff, 0xbe, 0x31, 0x7c, 0xe8, 0xba, 0xff, 0xe8, 0xc6, 0xff, 0xe8, 0xc3, 0xff, 0xe8, 0xce, 0xff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x55, 0xaa };
        ...
        else if (cmd.ToLower() == "key" || cmd.ToLower() == "key ")
        {
            if (string.IsNullOrWhiteSpace(args))
            {
                Console.BackgroundColor = ConsoleColor.Blue;
                Console.WriteLine("USAGE: key <key layout name>");
                Console.WriteLine("Key layouts :");
                Console.WriteLine("en : English QWERTY keyboard");
                Console.WriteLine("de : Deutsch QWERTZ keyboard");
                Console.WriteLine("fr : French AZERTY keyboard");
                Console.BackgroundColor = ConsoleColor.Black;
            }
            else
            {
                //fd.Delete("/etc/conf/keylayout.fc");
                switch (args)
                {
                    case "en":
                        dewitcher.Core.KeyLayout.QWERTY();
                        FS.AddFile("en", "keylayout.fc", "/etc/conf");
                        break;
                    case "de":
                        dewitcher.Core.KeyLayout.QWERTZ();
                        FS.AddFile("de", "keylayout.fc", "/etc/conf");
                        break;
                    case "fr":
                        dewitcher.Core.KeyLayout.AZERTY();
                        FS.AddFile("fr", "keylayout.fc", "/etc/conf");
                        break;
                    default:
                        Console.BackgroundColor = ConsoleColor.Red;
                        Console.WriteLine("CAN'T PARSE ARGUMENT : " + args);
        ...
        else if (cmd.ToLower() == "del" || cmd.ToLower() == "del ")
        {
            fd.Delete(args);
        }
        /*
